...
class VoyagerMediaController extends Controller
{
                ...
                if ($request->file->getClientOriginalExtension() == 'gif') {
                    ...
                    if (property_exists($details, 'thumbnails') && is_array($details->thumbnails)) {
                        foreach ($details->thumbnails as $thumbnail_data) {
                            ...
                            if ($type == 'fit') {
                                $thumbHeight = $thumbnail_data->height ?? null;
                                $thumbPosition = $thumbnail_data->position ?? 'center';
                                if ($thumbHeight !== null) {
                                    $thumbnail->cover(
                                    ...
                                    $thumbnail->scaleDown($thumbnail_data->width, null);
                                }
                            } elseif ($type == 'crop') {
                                $thumbnail->crop(
                                    $thumbnail_data->width,
                                ...
                                $noUpsize = !($thumbnail_data->upsize ?? true);
                                if ($noUpsize) {
                                    $thumbnail->scaleDown(
                                        $thumbnail_data->width,
                            ...
                            Storage::disk($this->filesystem)->put($thumbnail_file, $thumbnail->encodeByExtension($extension, quality: ($details->quality ?? 90))->toString());
                        }
                    }
                    ...
                    Storage::disk($this->filesystem)->put($file, $image->encodeByExtension($extension, quality: ($details->quality ?? 90))->toString());
                }
            }
    ...
    public function crop(Request $request)
    {
        ...
        try {
            ...
            $ext = pathinfo($originImagePath, PATHINFO_EXTENSION);
            $image = Image::read($content)->crop($width, $height, $x, $y);
            Storage::disk($this->filesystem)->put($destImagePath, $image->encodeByExtension($ext)->toString());
    ...
    private function addWatermarkToImage($image, $options)
    {
        ...
        $watermark->scaleDown($width, null);
